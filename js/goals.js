(function(){
  ...
  async function saveLineup(item,pin){const remote=await request('saveLineup',{item,pin}),saved=remote?.item||item,items=localList(LINEUP_KEY),index=items.findIndex(x=>String(x.matchNumber)===String(saved.matchNumber));index>=0?items[index]=saved:items.push(saved);localStorage.setItem(LINEUP_KEY,JSON.stringify(items));window.dispatchEvent(new CustomEvent('chiqui:lineup-saved',{detail:saved}));return saved}
