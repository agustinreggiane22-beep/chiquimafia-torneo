(function(){
  ...
  async function results(){const jsonp=await jsonpResults().catch(()=>null);if(Array.isArray(jsonp?.results))return jsonp.results;const remote=await request('listResults').catch(()=>null);return remote?.items||localList(RESULTS_KEY)}
  ...
  async function champions(){const remote=await request('listChampions').catch(()=>null);return remote?.items||[]}
  ...
  window.ChiquiGoals={list,submit,decide,authenticate,approvedTotals,mvpAwards,confirmMvp,mvpTotals,deleteMvp,lineups,saveLineup,results,saveResult,sanctions,addSanction,deleteSanction,clearGoals,deleteSubmission,deletedDates,deleteDate,webPlayers,setWebPlayer,playoffResults,champions,savePlayoff,resetSeason,isShared:()=>Boolean(API())};
})();
