(function () {
  ...
  function uniqueNames(values){
    const byKey=new Map();
    values.map(clean).filter(Boolean).forEach(name=>{const id=key(name);if(id&&!byKey.has(id))byKey.set(id,name)});
    return [...byKey.values()];
  }
  function buildTournament(state){
    ...
    const matches=(state.lineups||[]).filter(x=>!deleted.has(String(x.matchNumber))).map(x=>{const result=resultMap.get(String(x.matchNumber));return{number:number(x.matchNumber),date:clean(x.date),white:uniqueNames(array(x.white)),black:uniqueNames(array(x.black)),result:result?result.winner==='draw'?'Empate':result.winner==='white'?'Gana E1':'Gana E2':'',winner:result?.winner||'',played:Boolean(result&&String(result.played)!=='false'),whiteGoals:result?number(result.whiteGoals):undefined,blackGoals:result?number(result.blackGoals):undefined,mvp:''}}).sort((a,b)=>a.number-b.number);
    const names=uniqueNames([...active,...matches.flatMap(match=>[...match.white,...match.black])]),canonical=new Map(names.map(name=>[key(name),name]));
    matches.forEach(match=>{match.white=uniqueNames(match.white).map(name=>canonical.get(key(name))||name);match.black=uniqueNames(match.black).map(name=>canonical.get(key(name))||name)});
    const rows=new Map(names.map(player=>[key(player),{rank:0,player,points:0,played:0,wins:0,draws:0,losses:0,mvp:0}])),mvpByMatch=new Map((state.mvps||[]).map(x=>[String(x.matchNumber),canonical.get(key(x.player))||clean(x.player)])),mvpBonuses=new Map(names.map(player=>[key(player),0])),historyMap=new Map(names.map(player=>[key(player),[]]));
    const regularSeasonRounds=Number(window.CHIQUI_CONFIG.regularSeasonRounds||18);
    matches.filter(match=>match.played&&Number(match.number)<=regularSeasonRounds).forEach(match=>{const whiteOutcome=match.winner==='draw'?'draw':match.winner==='white'?'win':'loss',blackOutcome=match.winner==='draw'?'draw':match.winner==='black'?'win':'loss';[[match.white,whiteOutcome],[match.black,blackOutcome]].forEach(([players,outcome])=>[...new Set(players.map(clean))].forEach(player=>{const row=rows.get(key(player));if(!row)return;row.played+=1;row.points+=Number(scoring.attendance||0);if(outcome==='win'){row.wins+=1;row.points+=Number(scoring.win||0)}else if(outcome==='draw'){row.draws+=1;row.points+=Number(scoring.draw||0)}else{row.losses+=1;row.points+=Number(scoring.loss||0)}}));const mvp=mvpByMatch.get(String(match.number));if(mvp){match.mvp=mvp;mvpBonuses.set(key(mvp),(mvpBonuses.get(key(mvp))||0)+Number(scoring.mvp||0))}names.forEach(player=>{const row=rows.get(key(player));historyMap.get(key(player)).push({date:'Fecha '+match.number,value:row.points+(mvpBonuses.get(key(player))||0)})})});
